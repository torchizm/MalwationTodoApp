...
const AddUser = () => {
    ...
    useEffect(() => {
        ...
        api.get(`workspace/${workspaceId}/member`).then(res => {
            ...
            <div className='content-actions'>
                {workspace.name !== undefined &&
                    <p className='text-clamp' style={{ fontWeight: '500', flexGrow: '1' }}>
                        <Link className='breadcrumb' to="/dashboard">Kontrol Paneli</Link> / <Link className='breadcrumb' to={`/workspace/${workspaceId}`}>{workspace.name}</Link> / Kullanıcı Ekle
                    </p>
                }
            </div>
